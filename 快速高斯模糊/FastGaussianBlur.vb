...
Public Class FastGaussianBlur
    ...
    Private ReadOnly TaskParalleOptions As ParallelOptions = New ParallelOptions With {.MaxDegreeOfParallelism = 8}
    ...
    Public Function GaussianBlur(Radius As Integer) As Bitmap
        ...
        Dim Dest(BitmapWidth * BitmapHeight - 1) As Integer
        ...
        Parallel.[For](0, Dest.Length, TaskParalleOptions,
            Sub(Index As Integer)
                If NewRed(Index) > 255 Then
                    NewRed(Index) = 255
                End If
                If NewGreen(Index) > 255 Then
                    NewGreen(Index) = 255
                End If
                If NewBlue(Index) > 255 Then
                    NewBlue(Index) = 255
                End If
                If NewRed(Index) < 0 Then
                    NewRed(Index) = 0
                End If
                If NewGreen(Index) < 0 Then
                    NewGreen(Index) = 0
                End If
                If NewBlue(Index) < 0 Then
                    NewBlue(Index) = 0
                End If
                Dest(Index) = (-16777216 Or NewRed(Index) << 16 Or NewGreen(Index) << 8 Or NewBlue(Index))
            End Sub)
        ...
        Marshal.Copy(Dest, 0, BiamapBits.Scan0, Dest.Length)
        NewBitmap.UnlockBits(BiamapBits)
        GC.SuppressFinalize(Me)
        Return NewBitmap
    End Function

    Private Sub ShellBlur(Source As Integer(), Dest As Integer(), Radius As Integer)
        Dim Boxs As Integer() = BoxesForGauss(Radius, 3)
        BoxBlur(Source, Dest, BitmapWidth, BitmapHeight, (Boxs(0) - 1) / 2)
        BoxBlur(Dest, Source, BitmapWidth, BitmapHeight, (Boxs(1) - 1) / 2)
        BoxBlur(Source, Dest, BitmapWidth, BitmapHeight, (Boxs(2) - 1) / 2)
    End Sub
    ...
    Private Sub BoxBlur(Source As Integer(), Dest As Integer(), Width As Integer, Height As Integer, Radius As Integer)
    ...
    Private Sub BoxBlurH(Source As Integer(), Dest As Integer(), Width As Integer, Height As Integer, Radius As Integer)
        ...
        Parallel.[For](0, Height, TaskParalleOptions,
            Sub(Index As Integer)
                ...
                For Index = 0 To Radius
                    ...
                    Dest(CountInLine) = CInt(Math.Round(CDec(TempValue) * IARadius))
                    LineLength += 1 : CountInLine += 1
                Next
                For Index = Radius + 1 To Width - Radius - 1
                    TempValue = TempValue + (Source(LineLength) - Dest(LengthLine))
                    Dest(CountInLine) = CInt(Math.Round(CDec(TempValue) * IARadius))
    ...
    Private Sub BoxBlurT(Source As Integer(), Dest As Integer(), Width As Integer, Height As Integer, Radius As Integer)
        Dim IARadius As Double = 1.0 / CDec((Radius + Radius + 1))
